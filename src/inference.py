...
def fetch_next_hour_predictions():
    ...
    print(f"Next hour target: {next_hour}")
    print(f"Predicted records found: {len(df)}")
    return df
...
def fetch_hourly_rides(hours):
    ...
    return fg.select_all().filter(fg.pickup_hour >= current_hour).read()
...
def fetch_days_data(days):
    ...
    print(f"Fetching historical range: {fetch_data_from} to {fetch_data_to}")

    fs = get_feature_store()
    fg = fs.get_feature_group(name=config.FEATURE_GROUP_NAME, version=1)
    df = fg.read()

    return df[(df["pickup_hour"] >= fetch_data_from) & (df["pickup_hour"] <= fetch_data_to)]
